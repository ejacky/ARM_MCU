...
void PORTA_IRQHandler(void)
{
  uint8_t  n = 0;    //引脚号
  n = 1;
  if(PORTA_ISFR & (1 << n))         //PTA1触发中断
  {
    PORTA_ISFR  |= (1 << n);        //写1清中断标志位
    uart_sendStr(UART1,"UP_Key Down!\r\n"); // 发送 LEFT_Key Down 提示此按键被按下   
  }
  n = 12;
  if(PORTA_ISFR & (1 << n))         //PTA12触发中断
  {
    PORTA_ISFR  |= (1 << n);        //写1清中断标志位
    uart_sendStr(UART1,"LEFT_Key Down!\r\n"); // 发送 RIGHT_Key Down 提示此按键被按下
  }  
  n = 5;
  if(PORTA_ISFR & (1 << n))         //PTA5触发中断
  {
    PORTA_ISFR  |= (1 << n);        //写1清中断标志位
    uart_sendStr(UART1,"DOWN_Key Down!\r\n"); // 发送 UP_Key Down 提示此按键被按下
  } 
  n = 13;
  if(PORTA_ISFR & (1 << n))         //PTA13触发中断
  {
    PORTA_ISFR  |= (1 << n);        //写1清中断标志位
    uart_sendStr(UART1,"RIGHT_Key Down!\r\n"); // 发送 DOWN_Key Down 提示此按键被按下
  }
  
  n = 14;
  if(PORTA_ISFR & (1 << n))         //PTA14触发中断
  {
    PORTA_ISFR  |= (1 << n);        //写1清中断标志位
    uart_sendStr(UART1,"A_Key Down!\r\n"); // 发送 A_Key Down 提示此按键被按下
  } 
  n = 16;
  if(PORTA_ISFR & (1 << n))         //PTA16触发中断
  {
    PORTA_ISFR  |= (1 << n);        //写1清中断标志位
    uart_sendStr(UART1,"B_Key Down!\r\n"); // 发送 B_Key Down 提示此按键被按下
  }
   n = 17;
  if(PORTA_ISFR & (1 << n))         //PTA17触发中断
  {
    PORTA_ISFR  |= (1 << n);        //写1清中断标志位
    uart_sendStr(UART1,"C_Key Down!\r\n"); // 发送 C_Key Down 提示此按键被按下
  }
   n = 15;
  if(PORTA_ISFR & (1 << n))         //PTA15触发中断
  {
    PORTA_ISFR  |= (1 << n);        //写1清中断标志位
    uart_sendStr(UART1,"D_Key Down!\r\n"); // 发送 D_Key Down 提示此按键被按下
  }
}
