...
void AngleAcceleration_Digital_init(void)
{
  ...
  gpio_init (PTB11, GPO,HIGH);    //初始化I2C时钟引脚
...
void AngleAcceleration_Digital_Data(uint16_t *AdColle)
{
  ...
  { 
    ...
    for(int i=0;i<3;i++)
    {
      AdColle[i]=( XYZ_Data[i*2]<<8|XYZ_Data[i*2+1] )>>4;    //MMA8451 AD采集14位  向右移4位，数据位一个12位的AD采集精度 
      if(AdColle[i]>=0x0800)AdColle[i]= 0 - ( ( ~AdColle[i] )&0x07ff );
...
uint8 AngleAcceleration_AD(uint16_t *AdColle,uint8_t Count)
{
   ...
   AdColle[3] = ad_ave(ADC0_SE14, ADC_10bit, ADCOUT); //ZG源程序 PTC0
   AdColle[4] = ad_ave(ADC0_SE15, ADC_10bit, ADCOUT); //YG源程序 PTC1
   AdColle[5] = ad_ave(ADC0_SE11, ADC_10bit, ADCOUT); //xG源程序 PTC2
