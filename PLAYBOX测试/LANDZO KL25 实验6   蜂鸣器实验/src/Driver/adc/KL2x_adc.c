...
void adc_DMA_init(ADC_Ch ch)
{
    adc_init(ch)  ;
    //初始化ADC默认配置
    ADC_CFG1_REG(ADC0_BASE_PTR)  = ADLPC_LOW
                                 | ADC_CFG1_ADIV(ADIV_4)
                                 | ADLSMP_SHORT
                                 | ADC_CFG1_MODE(0)                   //8位
                                 | ADC_CFG1_ADICLK(ADICLK_BUS);
    ADC_CFG2_REG(ADC0_BASE_PTR)  = (ch>>5) // MUXSEL_ADCB    //MUXSEL_ADCA
                                 | ADACKEN_ENABLED
                                 | ADHSC_HISPEED
    ...
    ADC_SC2_REG(ADC0_BASE_PTR) = ADTRG_SW                       //硬件触发
                                | ACFE_DISABLED 
                                | ACFGT_GREATER 
                                | ACREN_DISABLED 
    ...
    ADC_SC1_REG(ADC0_BASE_PTR,ch>>5) = AIEN_ON | DIFF_SINGLE |      //
                                 ADC_SC1_ADCH( ch );
...
void adc_init(ADC_Ch ch)
{
  ...
  switch(ch)
  {
    case ADC0_SE4a:   //ADC0_SE4a
      Port_alt(PTE21,ALT0); //ADC0_SE4a -- PTE21
      break;  
    case ADC0_SE4b:   //ADC0_SE4b
      Port_alt(PTE29,ALT0); //ADC0_SE4b -- PTE29
      break;    
    ...
    case ADC0_SE23:   //ADC0_SE23 -- PTE30
      Port_alt(PTE30,ALT0);
      break;
    case ADC0_SE0:   //ADC0_SE0 -- PTE20
      Port_alt(PTE20,ALT0);
      break;
    case ADC0_SE3:   //ADC0_SE3 -- PTE23
      Port_alt(PTE22,ALT0);
      break;
    default:
          break;
...
uint16_t ad_once(ADC_Ch ch, ADC_nbit bit) //采集某路模拟量的AD值
{
    ...
    while (( ADC_SC1_REG(ADC0_BASE_PTR, 0) & ADC_SC1_COCO_MASK ) != ADC_SC1_COCO_MASK);
    result = ADC_R_REG(ADC0_BASE_PTR, 0);
...
void adc_start(ADC_Ch ch, ADC_nbit bit)
{
  ...
  ADC_CFG1_REG(ADC0_BASE_PTR)  = ADLPC_NORMAL
                               ...
                               | ADC_CFG1_MODE(bit)
                               | ADC_CFG1_ADICLK(ADICLK_BUS);
  ADC_CFG2_REG(ADC0_BASE_PTR)  =  ((ch>>5)  << ADC_CFG2_MUXSEL_SHIFT)   //MUXSEL_ADCB    //MUXSEL_ADCA
                               | ADACKEN_DISABLED
                               | ADHSC_HISPEED
                               | ADC_CFG2_ADLSTS(ADLSTS_20) ;
  ADC_SC1_REG(ADC0_BASE_PTR,0) = AIEN_ON | DIFF_SINGLE | ADC_SC1_ADCH( ch );
